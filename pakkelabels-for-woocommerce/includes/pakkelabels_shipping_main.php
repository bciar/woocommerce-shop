 ...
 */
abstract class Pakkelabels_Shipping_Main extends WC_Shipping_Method
{
    ...
    function init() {
        ...
        if (!class_exists('Pakkelabels_Shipping_Admin'))
        {
            include_once('pakkelabels_shipping_admin.php');
            $oPakkelabels_Shipping_Admin = new Pakkelabels_Shipping_Admin($this);
        }
    } // function init ends
        ...
        if (!empty($zipcode) && is_numeric($zipcode) && !empty($frontend_key))
        {
            ...
            if (empty($tempShopList['message']) || $tempShopList['message'] == null)
            {
                if (!empty($tempShopList))
                {
                    ...
                    ob_start();
                    ?>
                    <div class="pakkelabels-shoplist" data-pakkelabels_shipping_type="<?php print $agent; ?>">
                        <ul class="pakkelabels-shoplist-ul">
                            <?php
                            foreach ($tempShopList as $shop)
                            {
                                ?>
                                <li data-shopid="<?php echo $shop->number; ?>" class="pakkelabels-shop-list" data-pakkelabels_shipping_type="<?php print $agent; ?>">
    ...
    public function calculate_shipping( $package = array() )
    {
        ...
        else
        {
            ...
            {
                $oRows = get_option('Price_'.$this->instance_id);
                foreach ($oRows as $oRow)
                {
                    if((float)$oRow->minimum < (float)$iCart_total && (float)$iCart_total <= (float)$oRow->maximum)
                    {
            ...
            else if ($this->get_option('differentiated_price_type') == "Weight")
            {
                ...
                $oRows = get_option('Weight_'.$this->instance_id);
                foreach ($oRows as $oRow)
                {
            ...
            //If everything fails, defaults shipping price is set to 0 - instead of failure
            // if(!isset($return_price))
            //{
            //   $return_price = 0;
            //]
